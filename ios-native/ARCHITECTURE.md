...
|---|---|---|---|
| 1 | Feedback on pointerdown, before commit | `ALUI/Interaction/TouchDown.swift` + pre-warmed `AudioEngine` | `TouchDownTests` (recogniser fires on `.began`); `AudioLatencyTests` (no decode on the tap path); every `KVStore`/`ProfileStore` method is synchronous — an async adapter cannot conform |
...
| 6 | Accessibility floor | `Tile` 92pt floor, `.accessibilityLabel` from `Copy.swift`, `ReduceMotionSource` gating every `Anim.*` and `ConfettiSystem.fire()` | `AccessibilityTests`; `faceLabel` unit-tested in ALCore |
