...
|---|---|---|---|
| 1 | Feedback on pointerdown, before commit | `ALUI/Interaction/TouchDown.swift` + pre-warmed `AudioEngine`; **no `ScrollView` over an EXERCISE tile grid** — `delaysContentTouches` holds the touch for pan recognition, so `PageScroll` is gated per branch (D42, D46). The shop and the species picker do scroll `LayerHost` cards; that cost is unmeasured and flagged, not waived | `TouchDownTests` (recogniser fires on `.began`); `AudioLatencyTests` (no decode on the tap path); `PageScrollTests` (`enabled: false` is the pixel-exact identity); every `KVStore`/`ProfileStore` method is synchronous — an async adapter cannot conform |
