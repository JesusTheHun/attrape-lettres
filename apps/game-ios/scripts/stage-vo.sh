...
# committed.
#
# What IS committed is `Sources/ALPlatform/Resources/vo-manifest.txt` — one
# `<voKey>.<ext>` per line. That is what makes the coverage test
# ("every utterance the app can speak has a clip") run on a machine that has
# never staged the audio, which is the whole point: CI catches a new word
# shipping without a voice.
#
# Usage:
#   scripts/stage-vo.sh                # hard-link (default; instant, 0 bytes)
#   scripts/stage-vo.sh --copy         # real copies (for archiving an .ipa)
